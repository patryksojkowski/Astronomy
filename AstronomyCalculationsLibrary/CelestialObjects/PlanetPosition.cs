...
namespace AstronomyCalculationsLibrary.CelestialObjects
{
  ...
  {
    public PlanetPosition(string name, Func<decimal, decimal> ascedningNode, Func<decimal, decimal> inclination,
      Func<decimal, decimal> peryhelion,
    ...
    public override Vector2<decimal> EclipticCoordinates(DateTime date)
    {
      ...
      var lat = DecimalMath.Asin(pos.Z / DecimalMath.Sqrt(pos.X * pos.X + pos.Y * pos.Y + pos.Z * pos.Z)) *
                DecimalMath.RadToDeg;
      return new Vector2<decimal>(lon.From0To360(), lat);
    }
...
#if DEBUG
      Console.WriteLine(
        $"Geocentric ecliptic position: X = {pos.X - earthPos.X}, Y = {pos.Y - earthPos.Y}, Z = {pos.Z - earthPos.Z}");
#endif
    ...
    public override Vector3<decimal> EclipticSun3D(DateTime date)
    {
      ...
      Console.WriteLine(
        $"Peryhelion: {Peryhelion(days)} true anomaly : {DecimalMath.Atan2(Yv, Xv) * DecimalMath.RadToDeg}");
#endif
