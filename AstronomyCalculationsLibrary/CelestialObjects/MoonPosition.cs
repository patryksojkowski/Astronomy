...
namespace AstronomyCalculationsLibrary.CelestialObjects
{
    internal class MoonPosition : ICelestialPosition
    {
        ...
        private static readonly Func<decimal, decimal>
            AL = T => 3958M * DecimalMath.Sin(A1rad(T)) + 1962M * DecimalMath.Sin((FL(T) - FF(T)) * DecimalMath.DegToRad) + 318M * DecimalMath.Sin(A2rad(T));
        ...
        private static readonly Func<decimal, PeriodicTerm, decimal>
            Fs = (T, terms) =>
            {
                return terms.d * ToRad(FD(T)) + terms.m * ToRad(FMSun(T)) + terms.m1 * ToRad(FMMoon(T)) + terms.f * ToRad(FF(T));
            };

        private static List<PeriodicTerm> terms = new List<PeriodicTerm>();
