...
namespace AstronomyCalculationsLibrary
{
    ...
    public static class AstrologyCalculations
    {
        ...
        /// <returns></returns>
        public static void GetHousesAndDegrees(DateTime dateTime, double longitude, double latitude, out List<int[]> HousesPlanets, out List<Pair<int, decimal>> PlanetDegrees,
            bool isVedic = true)
        {
            PlanetManager.GetInstance().CalculatePositions(dateTime, longitude, latitude, isVedic, out HousesPlanets, out PlanetDegrees);
        }
